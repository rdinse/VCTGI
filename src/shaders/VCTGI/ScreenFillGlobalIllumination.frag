...
vec4 traceCone(vec3 pos, vec3 dir, float aperture, bool aoEnabled, float coneWeight) {
  ...
  while (coneSample.a <= 1.0 && dst <= maxTracingDistance) {
    ...
    //! Disabled with false &&
    if (false && !aoEnabled && aperture < 0.3 && emptyCount > 1 && interpolatedSample.a < 0.01 && mipLevel < uniformVoxelDepth - 3) {
      ...
      */
      ...
      while (maxLevel > 0 && texelFetch(voxelTex[0], maxVoxelPosI, maxLevel).a > 0.01) {
        //while (maxLevel > 0 && textureLod(voxelTex[visibleFace.x], samplePos, maxLevel).a > 0.01) {
        maxLevel--;
        maxLevelRes <<= 1;
      ...
      //maxLevelRes = 1 << (uniformVoxelDepth - maxLevel);
      //maxVoxelPosI = min(ivec3(maxLevelRes * samplePos), ivec3(maxLevelRes - 1));
      ...
      */
      ...
      samplingStepSize = max(tFar + voxelSize / 2.0, voxelSize);
      ...
      samplingStepSize = max(diameter/2, voxelSize);
    }

    dst += samplingStepSize;
    diameter = dst * aperture;
    samplePos = dir * dst + pos;
  }

  if (aoEnabled) {
    ambientOcclusion -= coneWeight * vec4(aoSample);
  }
...
void main() {
  ...
  pos += 2.5 * normal.xyz / uniformVoxelRes;
  ...
  if (uniformSpecularAlpha > 0.0) {
    ...
    specular = traceCone(pos, specularDir, uniformSpecularAperture, false, 1);
  }
  ...
  diffuse +=         traceCone(pos, normal.xyz,                                             2 * 0.5,      true, 1);
  diffuse += 0.607 * traceCone(pos, tangentSpaceMat * vec3(-0.794654, 0.607062,  0.000000), 2 * 0.549092, true, 0.607);
  diffuse += 0.607 * traceCone(pos, tangentSpaceMat * vec3( 0.642889, 0.607062,  0.467086), 2 * 0.549092, true, 0.607);
  diffuse += 0.607 * traceCone(pos, tangentSpaceMat * vec3( 0.642889, 0.607062, -0.467086), 2 * 0.549092, true, 0.607);
  diffuse += 0.607 * traceCone(pos, tangentSpaceMat * vec3(-0.245562, 0.607062,  0.755761), 2 * 0.549092, true, 0.607);
  diffuse += 0.607 * traceCone(pos, tangentSpaceMat * vec3(-0.245562, 0.607062, -0.755761), 2 * 0.549092, true, 0.607);
